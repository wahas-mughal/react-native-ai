...
The system SHALL send the user's message to Apple Foundation Models via the Vercel AI SDK using `@react-native-ai/apple` and display the assistant's response in the chat.
...
- **AND** the system calls `streamText` (or `generateText`) with `model: apple()` and the conversation history
